...
export const loginUser = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Send login notification email AFTER response (non-blocking)
    ...
    setImmediate(async () => {
      try {
        ...
        if (emailSent) {
          ...
          console.log(`⚠️ Failed to send login notification email to ${user.email} (non-critical)`);
        }
      } catch (emailError) {
        console.error('⚠️ Error sending login notification email (non-critical):', emailError);
        // Already sent response, so don't do anything
      }
    });
