...
class BlockedDatesAdmin {
  ...
  async getSummary() {
    ...
    const summary = {
      ...
      blockedDates: allBlockedDates.map((bd: BlockedDate) => ({ 
        date: bd.date, 
        reason: bd.reason 
      })),
  ...
  async validateConsistency() {
    ...
    allBlockedDates.forEach((bd: BlockedDate) => {
  ...
  async blockMultipleDates(dates: string[], reason: string = 'Day blocked off') {
